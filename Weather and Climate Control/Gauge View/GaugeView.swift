...
struct GaugeView: View {
    ...
    var startPoint: UnitPoint {
        let offset = (gaugeData.minimunValue - gaugeData.minimunTrackValue) / (gaugeData.maximimTrackValue - gaugeData.minimunTrackValue)
        return UnitPoint(
            x: offset,
            y: 0.5
        )
    ...
    var endPoint: UnitPoint {
        let offset = (gaugeData.maximimValue - gaugeData.minimunTrackValue) / (gaugeData.maximimTrackValue - gaugeData.minimunTrackValue)
        return UnitPoint(
            x: offset,
            y: 0.5
        )
